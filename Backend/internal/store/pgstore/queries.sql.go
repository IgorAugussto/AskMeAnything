...
const getRoomeasaege = `-- name: GetRoomeasaege :many
SELECT
  "id", "room_id", "message", "reaction_count", "answered"
FROM messages
WHERE
...
func (q *Queries) GetRoomeasaege(ctx context.Context, roomID uuid.UUID) ([]Message, error) {
	rows, err := q.db.Query(ctx, getRoomeasaege, roomID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const markMessageAsAsnwered = `-- name: MarkMessageAsAsnwered :exec
UPDATE messages
SET
  answered = true
WHERE
...
func (q *Queries) MarkMessageAsAsnwered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markMessageAsAsnwered, id)
	return err
}
